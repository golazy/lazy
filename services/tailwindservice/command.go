...
func RequiredPackages() []string {
	return append([]string(nil), requiredPackages...)
}
...
func (c Command) Execute() (int, error) {
	...
	packageManager := nodeservice.DetectNodePackageManager(root, nodeservice.QueryRunner(c.Runner, c.Mise))
	...
	if err := runner(installCommand, installExecArgs, execservice.Options{
		Dir:    root,
		Env:    installEnv,
		Stdout: stdout,
		Stderr: stderr,
	...
	if err := runner(tailwindCommand, tailwindArgs, execservice.Options{
		Dir:    root,
		Env:    tailwindEnv,
		Stdout: stdout,
		Stderr: stderr,
...
func tailwindRunCommand(packageManager nodeservice.NodePackageManager, root string, inputPath string, outputPath string, watch bool) (string, []string) {
	args := []string{
...
func EnsurePackageDevDependencies(path string, packages []string) (bool, error) {
	return ensurePackageDevDependencies(path, packages)
}
