...
func ParseManifest(data []byte) (Manifest, error) {
		...
		if strings.HasPrefix(line, "[") {
			...
			if strings.HasPrefix(section, "entrypoint.") {
				...
				if _, ok := entryByName[name]; !ok {
					manifest.Entrypoints = append(manifest.Entrypoints, Entrypoint{Name: name})
	...
	if len(manifest.Entrypoints) == 0 {
		return Manifest{}, fmt.Errorf("at least one [entrypoint.<name>] block is required")
	}
	for _, entrypoint := range manifest.Entrypoints {
		if entrypoint.Module == "" {
			return Manifest{}, fmt.Errorf("entrypoint %q is missing module", entrypoint.Name)
		}
	}
	return manifest, nil
}
