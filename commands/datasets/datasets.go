...
import (
	...
	"golazy.dev/lazy/commands/services"
)
...
func (c Command) Dump(name string) error {
	...
	inventory, err := services.Inspect(dir, config)
	if err != nil {
		return err
	}
	...
	for _, service := range inventory.Services {
		if services.HasTask(inventory.Tasks, service, "dump") {
	...
	for _, service := range dumpable {
		...
		if err := services.RunTask(c.Runner, dir, services.TaskName(service, "dump"), []string{path}, c.Stdin, c.Stdout, c.Stderr, false); err != nil {
...
func (c Command) Load(name string) error {
	...
	for _, service := range inventory.Services {
		...
		if !services.HasTask(inventory.Tasks, service, "load") {
		...
		if err := services.RunTask(c.Runner, dir, services.TaskName(service, "load"), []string{path}, c.Stdin, c.Stdout, c.Stderr, false); err != nil {
