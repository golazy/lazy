...
type Command struct {
	...
	OutputRunner execservice.OutputRunner
...
func (c Command) ExecuteDev() (int, error) {
	...
	if c.ViewPath != "" && c.ViewPath != appservice.DefaultViewPath {
		args = append(args, "--viewpath", filepath.ToSlash(c.ViewPath))
	}
	if c.PublicPath != "" && c.PublicPath != appservice.DefaultPublicPath {
		args = append(args, "--publicpath", filepath.ToSlash(c.PublicPath))
	}

	if err := c.runner()(helper, args, execservice.Options{
		Dir:    dir,
		Stdin:  c.Stdin,
		Stdout: c.Stdout,
		Stderr: c.Stderr,
...
func (c Command) ExecuteBuild() (int, error) {
	...
	if err := c.runner()("go", appservice.GoBuildArgs("", filepath.ToSlash(candidate), appBinary), execservice.Options{
		Dir:    dir,
		Stdout: c.Stdout,
		Stderr: c.Stderr,
	}); err != nil {
...
func (c Command) latestCommit() (string, error) {
	output, err := c.outputRunner()("git", []string{"ls-remote", helperRepo, "HEAD"}, execservice.Options{
		Stderr: io.Discard,
	})
	if err != nil {
		return "", err
...
func (c Command) installHelper(commit string, binary string) error {
	...
	if err := c.runner()("git", []string{"clone", "--depth", "1", helperRepo, src}, execservice.Options{
		Stdout: c.Stdout,
		Stderr: c.Stderr,
	}); err != nil {
	...
	if commit != "" {
		if err := c.runner()("git", []string{"checkout", commit}, execservice.Options{
			Dir:    src,
			Stdout: c.Stdout,
			Stderr: c.Stderr,
		}); err != nil {
	...
	if err := c.runner()("go", []string{"build", "-o", binary, "."}, execservice.Options{
		Dir:    src,
		Stdout: c.Stdout,
		Stderr: c.Stderr,
	}); err != nil {
...
func (c Command) outputRunner() execservice.OutputRunner {
	if c.OutputRunner != nil {
		return c.OutputRunner
	}
	return execservice.ExecOutput
}
