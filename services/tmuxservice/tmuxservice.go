...
func (c Command) tmux(runner execservice.Runner, args ...string) error {
	return runner("mise", append([]string{"exec", "--", "tmux"}, args...), execservice.Options{
		Dir:    c.Dir,
		Stdout: c.Stdout,
		Stderr: c.Stderr,
	})
...
func (c Command) tmuxQuiet(runner execservice.Runner, args ...string) error {
	return runner("mise", append([]string{"exec", "--", "tmux"}, args...), execservice.Options{
		Dir:     c.Dir,
		Stdout:  io.Discard,
		Stderr:  io.Discard,
...
func (c Command) tmuxAttach(runner execservice.Runner, session string) error {
	return runner("mise", []string{"exec", "--", "tmux", "attach-session", "-t", session}, execservice.Options{
		Dir:    c.Dir,
		Stdin:  c.Stdin,
		Stdout: c.Stdout,
		Stderr: c.Stderr,
...
func appCommand(session string, cmdPath string, viewPath string, publicPath string) string {
	...
	if viewPath != "" && viewPath != appservice.DefaultViewPath {
		parts = append(parts, "--viewpath", shellQuote(filepath.ToSlash(viewPath)))
	}
	if publicPath != "" && publicPath != appservice.DefaultPublicPath {
		parts = append(parts, "--publicpath", shellQuote(filepath.ToSlash(publicPath)))
	}
	return strings.Join(parts, " ")
}

func servicePreparedAppCommand(services []configservice.Service, session string, cmdPath string, viewPath string, publicPath string) string {
...
func ServicePreparedAppCommand(services []configservice.Service, session string, cmdPath string, viewPath string, publicPath string) string {
	return servicePreparedAppCommand(services, session, cmdPath, viewPath, publicPath)
}

func serviceSetupCommand(services []configservice.Service) string {
	if len(services) == 0 {
		return ""
	}
	parts := []string{
