...
func TestIndexProxiesApplicationControlPlaneForJSON(t *testing.T) {
	...
	appControl := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = fmt.Fprint(w, `{"directory":".tmp/traces","traces":[{"request_id":"req-123","path":"/pools"}]}`)
	}))
	...
	request := httptest.NewRequest(http.MethodGet, "/_golazy/traces", nil)
	...
	if !strings.Contains(response.Body.String(), `"request_id":"req-123"`) {
		t.Fatalf("body = %s, want trace JSON", response.Body.String())
	}
}
