...
- The in-page development panel launcher now uses the padded, yellow-backed
  GoLazy square logo in a rounded-square button, opens the panel on click, and
  the extension content bridge re-announces itself when the page host client
  becomes ready so the launcher stays hidden while the Chrome extension is
  active.
